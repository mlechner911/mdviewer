...
## MLC Doc Hub — marksafe

Structured documentation lives in `.mlcai/`, maintained through the
`mlc-dochub` MCP server.

- **Project ID:** `marksafe` — MarkSafe — Markdown Viewer & Editor
- **Source directory:** `/mnt/data2tb/marksafe` (read source files with native Read —
...
- **Use the project id `marksafe`.** Never invent `marksafe-mac`, and
